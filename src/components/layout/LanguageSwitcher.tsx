...
export function LanguageSwitcher() {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute end-0 top-full mt-1.5 min-w-[140px] overflow-hidden rounded-xl border border-[#E4E4E7] bg-white p-1 shadow-lg shadow-black/5">
          {languages.map((lang) => (
            <button
              key={lang.code}
              onClick={() => switchLanguage(lang.code)}
              className={`flex w-full items-center rounded-lg px-3 py-2 text-[13px] font-medium transition-colors ${
